...
static void traceEnable(struct Cora *co, int label, Obj *R) {
  str filePath = stringStr(R[1]);
  traceInit(filePath.str);
  coraReturn(co, True);
}

static void traceDisable(struct Cora *co, int label, Obj *R) {
  traceClose();
  coraReturn(co, True);
}

static void traceWrap(struct Cora *co, int label, Obj *R) {
...
static void builtinTrace(struct Cora *co, int label, Obj *R) {
  Obj sym = R[1];
...
static void builtinUntrace(struct Cora *co, int label, Obj *R) {
  Obj sym = R[1];
