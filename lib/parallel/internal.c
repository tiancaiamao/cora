...
startRoutine(void* arg) {
	...
	struct scmNative *ptr = (struct scmNative *)arg;
	// TODO: memory of arg belongs to another thread, unsafe!
	co->nargs = 1;
	co->args[0] = ((Obj) ptr | TAG_PTR);
...
newProc(struct Cora *co) {
  Obj arg = co->args[1];
  struct scmNative* proc = mustNative(arg);
  pthread_t thread;
  int ret = pthread_create(&thread, NULL, startRoutine, proc);
  if (ret != 0) {
	  coraReturn(co, Nil);
	  return;
  }
...
/* static void */
/* mailboxRecvBlock(struct Cora *co) { */
/* 	struct mailbox *m = mustC(co->args[1]); */
/* 	Obj k = co->args[2]; */
/* 	Obj wrap = co->args[3]; */
/* 	int fd = eventfd(0, EFD_NONBLOCK | EFD_CLOEXEC); */
...
/* 	/\* struct nodeBlocked *n = malloc(sizeof(struct nodeBlocked)); *\/ */
/* 	/\* n->fd = fd; *\/ */
/* 	/\* n->k = k; *\/ */
/* 	/\* n->wrap = wrap; *\/ */

/* 	// add self to event loop so others can awake us. */
/* 	// and also enqueue to mailbox blocked queue so send to mailbox trigger the awaking. */
/* 	/\* pollReadAddPtr(n); *\/ */
/* 	/\* enqueueBlocked(&m->blocked, n); *\/ */
