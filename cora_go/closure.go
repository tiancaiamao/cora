...
func compile(exp Obj, env *CompileEnv, tail bool, freeVars map[Obj]PosRef) func(env Env) {
	...
	if IsSymbol(pair.car) {
		...
		case symLambda: // (lambda (x y) x)
			...
			return CompileLambda(args, body, env, freeVars, compile)
...
func CompileLambda(args, body Obj, env *CompileEnv, freeVars map[Obj]PosRef,
	compile func (exp Obj, env *CompileEnv, tail bool, freeVars map[Obj]PosRef) func(env Env)) func(env Env) {
	env1 := &CompileEnv{
		parent: env,
		args:   args,
	}
