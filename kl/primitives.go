...
var allPrimitives []*ScmPrimitive = []*ScmPrimitive{
	...
	&ScmPrimitive{scmHead: scmHeadPrimitive, Name: "car", Required: 1, Function: PrimHead},
	&ScmPrimitive{scmHead: scmHeadPrimitive, Name: "cdr", Required: 1, Function: primTail},
...
func init() {
	for _, prim := range allPrimitives {
		s := MakeSymbol(prim.Name)
		mustSymbol(s).value = Obj(unsafe.Pointer(&prim.scmHead))
	}
}
...
func PrimValue(key Obj) Obj {
	...
	return MakeError(fmt.Sprintf("variable %s not bound", sym.str))
}
