...
func init() {
	...
	primitives = make(map[string]primitiveDesc)
...
func (i *instrPrimitive) Exec(vm *VM) {
	i.desc.fn(vm)
	if i.next == nil {
		vm.ret(vm.val)
	} else {
...
func closureConvert(exp Obj, locals Obj, env Obj, frees []Obj, nlets int) (Obj, []Obj, int) {
	...
	if sym, ok := car(exp).(*Symbol); ok {
		if desc, ok := primitives[sym.str]; ok {
			nargs := listLength(cdr(exp))
			if nargs > desc.required {
				panic("primitive argument mismatch")
			}
			if nargs < desc.required {
				l := tempList(desc.required - nargs)
				body := listAppend(exp, l)
				expr := cons(symLambda, cons(l, cons(body, Nil)))
				return closureConvert(expr, locals, env, frees, nlets)
			}
		}
	}
...
func compile(exp Obj, locals []Obj, frees Obj, next Instr) Instr {
	...
	return compileCall(exp, locals, frees, next)
}

func compileCall(exp Obj, locals []Obj, frees Obj, next Instr) Instr {
	if sym, ok := car(exp).(*Symbol); ok {
		if desc, ok := primitives[sym.str]; ok {
			args := cdr(exp)
			nargs := listLength(args)
			if nargs == desc.required {
				return compileList(args, locals, frees, &instrPrimitive{
					desc: desc,
					next: next,
				})
			}
