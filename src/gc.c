...
static struct heapArena *heapArenaNew() {
  TRACE_SCOPE("heapArenaNew");
  struct heapArena *ha = malloc(sizeof(struct heapArena));
  ha->next = NULL;
  ha->ptr = mmap(0, HEAP_ARENA_SIZE, PROT_READ | PROT_WRITE,
...
static bool heapArenaFull(struct heapArena *ha) {
  return ha->idx >= BLOCKS_PER_HEAP;
}

static struct Block *heapArenaAlloc(struct heapArena *h) {
...
static bool heapArenaContains(struct heapArena *h, void *p) {
...
static int smallSizeClassIdx[] = {0, 1, 1, 2, 3,  4,  5,  6,  7,  8,  8,  8, 8,
                                  9, 9, 9, 9, 10, 10, 10, 10, 10, 10, 10, 10};

static int mediumSizeClassIdx[] = {
    -1, -1, 9,  10, 11, 12, 12, 13, 13, 14, 14, 14, 14, 15, 15, 15, 15, 16,
    16, 16, 16, 16, 16, 16, 16, 17, 17, 17, 17, 17, 17, 17, 17, 18, 18, 18,
    18, 18, 18, 18, 18, 18, 18, 18, 18, 18, 18, 18, 18, 18, 18, 18, 18, 18,
    18, 18, 18, 18, 18, 18, 18, 18, 18, 18, 18, 18, 18, 18, 18};

static int getSlotBySize(int size) {
...
static int versionCmp(int v1, int v2) {
...
static void gcStatsReset(struct GCStats *stats) {
  stats->allocated = 0;
  stats->markSize = 0;
  stats->markCount = 0;
  stats->markSkip = 0;
...
struct GC {
  ...
  // This is a list of heapArena and if one heapArena is full, it's moved to
  // heap.
...
static void gcInuseSizeInc(GC *gc, int size) {
  assert(size > 0);
  gc->stats.markSize += size;
  gc->stats.markCount++;
}

static struct heapArena *gcGetHeapArena(GC *gc) {
...
static struct Block *blockNew(GC *gc) {
...
static void blockReset(GC *gc, struct Block *block) {
...
GC *gcInit() {
  ...
  gc->version =
      2; // Start from 2, so uninitialized blocks can be treated as garbage
  gc->rset = NULL;
  gc->freelist = NULL;
...
static struct heapArena *gcContains(GC *gc, void *p) {
...
static int inuse(GC *gc, scmHead *h) {
...
static struct Block *getBlock(GC *gc, int slot) {
...
bool gcTriggerCheck(GC *gc) {
  if (gc->stats.allocated > gc->stats.nextSize && gc->state == gcStateNone) {
...
// Used in allocDone
static void allocDone(GC *gc, int size, scmHead *ret) {
  ret->size = size;
  ret->version = (gc->state == gcStateNone)
...
static int getLargeObjSlots(scmHead *n) {
...
static void updateLargeObjectMeta(struct heapArena *ha, int start, int slots,
                                  scmHead *h) {
...
scmHead *heapArenaAllocLarge(struct heapArena *ha, GC *gc, int slots) {
  while (ha->curr + slots <= BLOCKS_PER_HEAP) {
    ...
    if (ha->curr == ha->idx) {
      ...
      struct scmFreeNode *n =
          (struct scmFreeNode *)(ha->ptr + (ha->curr * MEM_BLOCK_SIZE));
    ...
    while (slots1 < slots) {
      ...
      scmHead *next =
          (scmHead *)(ha->ptr + ((ha->curr + slots1) * MEM_BLOCK_SIZE));
      if (inuse(gc, next)) {
        break;
      }
    ...
    if (slots1 > slots) {
      scmHead *next =
          (scmHead *)(ha->ptr + ((ha->curr + slots) * MEM_BLOCK_SIZE));
...
static void *gcAllocLargeObject(GC *gc, int size) {
...
static scmHead *blockAlloc(GC *gc, struct Block *block) {
...
bool gcRegistForType(uint8_t idx, gcFunc fn) {
...
static void gcEnqueue(GC *gc, scmHead *p) {
...
static scmHead *gcDequeue(GC *gc) {
...
static scmHead *checkPointer(GC *gc, uintptr_t p, struct Block **block) {
  ...
  *block = b;
  ...
  /* if (from != addr) { */
  /* 	printf("WARNING: the ptr %p is inside scmHead object! {type=%d, size=%d,
...
static version_t nextVersion(int gen, uint64_t ver) {
...
static bool markObject(GC *gc, scmHead *from, version_t minv) {
...
void gcMark(GC *gc, uintptr_t p, version_t minv) {
  struct Block *b;
...
/* 		/\* printf("block %p, sizeClass%d, b->inuse=%d, actual
...
static void gcFlip(GC *gc) {
  ...
  if (coraDebug != NULL) {
    printf("gc%" PRIu64 "%s, trigger=%zusz, mark=%zusz %dcnt, skip=%dcnt, "
           "incremental=%zusz, freeBlock=%dcnt, sys=%zusz\n",
           gc->version, gen, gc->stats.nextSize, gc->stats.markSize,
           gc->stats.markCount, gc->stats.markSkip, gc->stats.allocated,
  ...
  int base =
      (gc->stats.youngGenSize + gc->stats.middleGenSize + gc->stats.oldGenSize);
  gc->stats.avgAllocated =
      (gc->stats.allocated + 3 * gc->stats.avgAllocated) / 4;

  /* printf("base=%d, avgAllocated=%d, prevTrigger=%d\n", base,
   * gc->stats.avgAllocated, gc->stats.prevTrigger); */

  gc->stats.nextSize = (int)(0.4 * (base + gc->stats.avgAllocated) +
...
static void gcRunIncremental(GC *gc) {
...
static void sweepSizeClass(GC *gc, struct runDoneProgress *pg) {
...
static void sweepLargeObjects(GC *gc, struct runDoneProgress *pg) {
  while (pg->ha != NULL) {
    int j = pg->ha->curr;
    while (j < pg->ha->idx) {
      ...
      if (p->type == scmHeadUnused) {
        ...
        slots = (p->size + MEM_BLOCK_SIZE - 1) / MEM_BLOCK_SIZE;
        if ((p->version & VERSION_MASK) == ((gc->version - 2) & VERSION_MASK)) {
          struct scmFreeNode *n = (struct scmFreeNode *)p;
          n->head.type = scmHeadUnused;
          n->slots = slots;
          updateLargeObjectMeta(pg->ha, j, slots, &n->head);
        }
      }
...
void gcRun(GC *gc) {
  ...
  // gcStateIncremental -> gcStateSweep: incremental GC done, wait for white to
  // be exhausted gcStateSweep -> gcFlip: now only ecru and black, flip
...
void writeBarrierForIncremental(GC *gc, uintptr_t *slot, uintptr_t val) {
...
static void gcRSet(GC *gc) {
  ...
  while (p != NULL) {
    scmHead *h = (scmHead *)p;
    if (versionCmp(gc->version & VERSION_MASK, h->version & VERSION_MASK) >=
        0) {
