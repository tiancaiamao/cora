...
struct gcQueueNode {
	struct gcQueueNode *next;
	char base[];
};

static struct gcQueueNode*
gcQueueNodeNew() {
	struct gcQueueNode *ret = (struct gcQueueNode *)malloc(MEM_BLOCK_SIZE);
	ret->next = NULL;
	return ret;
}

struct doubleLinkList {
	struct gcQueueNode *head;
	struct gcQueueNode *tail;
};
...
gcQueueInit(struct GC *gc) {
	struct gcQueueNode *b = gcQueueNodeNew();
	gc->gray.head = b;
	gc->gray.tail = b;
	gc->start = 0;
	gc->end = 0;
...
gcEnqueue(struct GC *gc, scmHead *p) {
	struct gcQueueNode *b = gc->gray.tail;
	if (b->base+gc->end + sizeof(scmHead *) > (char*)b + MEM_BLOCK_SIZE) {
		b = gcQueueNodeNew();
		gc->gray.tail->next = b;
		gc->gray.tail = b;
		gc->end = 0;
...
gcDequeue(struct GC *gc) {
	struct gcQueueNode *b = gc->gray.head;
	if (b->base + gc->start >= (char*)b + MEM_BLOCK_SIZE) {
		b = b->next;
		free(gc->gray.head);
		gc->start = 0;
		gc->gray.head = b;
