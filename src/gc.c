...
static int sizeClass[] =
  { 0, 16, 24, 32, 40, 48, 56, 64, 96, 128, 192, 256, 384, 512, 768, 1024,
  1536, 2048, MEM_BLOCK_SIZE
};

#define sizeClassSZ (sizeof(sizeClass) / sizeof(int))
static int smallSizeClassIdx[] =
  { 0, 1, 1, 2, 3, 4, 5, 6, 7, 8, 8, 8, 8, 9, 9, 9, 9, 10, 10, 10, 10, 10, 10,
  10, 10
};

static int largeSizeClassIdx[] =
  { -1, -1, 9, 10, 11, 12, 12, 13, 13, 14, 14, 14, 14, 15, 15, 15, 15, 16, 16,
...
checkPointer(struct GC *gc, uintptr_t p) {
  ...
  if (from != addr) {
    printf
      ("WARNING: the ptr %p is inside scmHead object! {type=%d, size=%d, version=%d}\n",
  ...
  // Not a cora Object?
  printf
    ("WARNING: some thing is wrong when checkPointer? %p {type=%d, size=%d, version=%d}\n",
