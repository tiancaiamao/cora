...
struct Block {
		...
		// Link the block into sizeClass list.
		struct Block *next;
		struct Block *prev;
};
...
struct GC {
		...
		// Small objects are allocated from the sizeClass blocks here.
		...
		// Large objects are allocated from *this heapArena* directly.
...
// This is the thread local object.
// According the design, each thread works with its own GC.
...
static void*
gcAllocLarge(struct GC *gc, int size) {
		...
		if (gc->large == NULL) {
				...
				gc->large = ha;
				// put the new heapArena to the second of gc->heap
				struct heapArena *heapNonEmpty = gcGetHeapArena(gc);
				ha->next = heapNonEmpty->next;
				heapNonEmpty->next = ha->next;
		}
...
gcAlloc(struct GC *gc, int size) {
		...
		// Large objects allocation goes to different code branch..
		if (size >= MEM_BLOCK_SIZE) {
				return gcAllocLarge(gc, size);
		}
...
gcDoneAndReset(struct GC *gc) {
		...
		for (struct heapArena* h=gc->heap; h != NULL; h = h->next) {
				for (int i=0; i<h->idx; i++) {
						...
						gc->sizeClass[slot] = b;
						printf("heap %p ith[%d] block offset after reset %d\n", h, i, b->curr);
				}
		}
}
