...
gcFlip(struct GC *gc) {
	...
	if (coraDebug != NULL) {
		printf("gc%" PRIu64
		       ", trigger size=%d, inuse=size(%d) count(%d), incremental size=%d, mark skip=%d, sys size=%zu\n",
		       gc->version, gc->nextSize, gc->inuseSize,
		       gc->inuseCount, gc->allocated, gc->markSkip, sysSize);
	}
