...
static void segmentStackAlloc(struct segmentStack *alloc) {
...
static void segmentStackInit(struct segmentStack *alloc) {
...
static void callCurry(Cora *co, int label, Obj *R) {
  Obj fn = R[0];
...
Obj makeCurry(Cora *co, Obj fn, int nargs, Obj *args) {
...
Binding bindSymbol(Cora *co, Obj sym) {
  strBuf s = ptr(sym);
  str xx = toStr(s);
...
void symbolToString(Cora *co, int lable, Obj *R) {
  Obj sym = R[1];
  char dest[256];
...
void builtinBytes(Cora *co, int label, Obj *R) {
...
static void builtinBytesLength(Cora *co, int label, Obj *R) {
  Obj o = R[1];
  int res = bytesLen(o);
  coraReturn(co, makeNumber(res));
}

void builtinIntern(Cora *co, int label, Obj *R) {
...
void builtinIsNumber(Cora *co, int label, Obj *R) {
  Obj x = R[1];
  if (isfixnum(x)) {
    coraReturn(co, True);
    return;
...
void builtinValue(Cora *co, int label, Obj *R) {
  Obj sym = R[1];
  Obj ret;
  if (tag(sym) == TAG_SYMBOL) {
...
void builtinValueOr(Cora *co, int label, Obj *R) {
  Obj sym = R[1];
  Obj ret;
  if (tag(sym) == TAG_SYMBOL) {
    ret = symbolGet(co, sym);
...
static void builtinApply(Cora *co, int label, Obj *R) {
...
static void builtinLoad(Cora *co, int label, Obj *R) {
...
static bool safeToUseSo(strBuf soFilePath) {
...
void builtinImport(Cora *co, int label, Obj *R) {
...
static void builtinIsVector(Cora *co, int label, Obj *R) {
...
static void builtinVector(Cora *co, int label, Obj *R) {
...
static void builtinVectorRef(Cora *co, int label, Obj *R) {
  Obj v = R[1];
  Obj idx = R[2];
  coraReturn(co, vectorRef(v, fixnum(idx)));
}

static void builtinVectorSet(Cora *co, int label, Obj *R) {
...
static void builtinVectorLength(Cora *co, int label, Obj *R) {
  Obj o = R[1];
  int res = vectorLength(o);
  coraReturn(co, makeNumber(res));
}

void builtinReadFileAsSexp(Cora *co, int label, Obj *R) {
  Obj arg = R[1];
  assert(isBytes(arg));
  Obj result = Nil;
...
static void builtinStringAppend(Cora *co, int label, Obj *R) {
...
static void builtinSymbolCooked(Cora *co, int label, Obj *R) {
  Obj sym = R[1];
  char dst[256];
...
static void builtinTryCatch(Cora *co, int label, Obj *R) {
...
Obj makeContinuation(Cora *co, Frame *callstack, int len, Obj **stk,
                     int count) {
...
static void continuationGCFunc(struct GC *gc, void *f) {
...
static void continuationAsClosure(Cora *co, int label, Obj *R) {
  Obj this = R[0];
...
static void builtinThrow(Cora *co, int label, Obj *R) {
...
Obj primEQ(Obj x, Obj y) { return eq(x, y) ? True : False; }
...
Obj primCons(Cora *co, Obj x, Obj y) { return makeCons(co->gc, x, y); }
...
Obj primCar(Obj x) { return car(x); }
...
static void applyClosureForEval(Cora *co, int label, Obj *R) {
  Obj self = R[0];
...
static void makeClosureForEval(Cora *co, int label, Obj *R) {
  Obj params = R[1];
  Obj body = R[2];
  Obj env = R[3];
...
static Obj primVMSymbolForTLS(Cora *co) {
  char dest[20];
...
static void vmSymbolForTLS(Cora *co, int label, Obj *R) {
  Obj ret = primVMSymbolForTLS(co);
  coraReturn(co, ret);
}

void registerAPI(Cora *co, struct registerModule *m, str pkg) {
  if (m->init != NULL) {
    m->init();
  }
...
void coraRegisterAPI(Cora *co, char *pkg, char *name, basicBlock func,
                     int argc) {
  Obj sym;
  if (pkg != NULL) {
...
static void coraGCFunc(GC *gc, void *ptr) {
