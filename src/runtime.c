...
// ============================================================================
// Cora VM Task Management API Implementation
// ============================================================================

#include <pthread.h>

// Internal structure to hold Cora VM with task management state
struct CoraVMWithTasks {
    Cora *cora_vm;                    // The underlying Cora VM instance
    
    // Task queues
    Obj ready_tasks;                  // List of ready tasks (cons cells)
    pthread_mutex_t task_queue_lock;  // Lock for thread-safe access
    
    // Statistics
    int total_tasks_spawned;          // Total number of tasks spawned
};

// Create a Cora VM instance with task management capabilities
CoraVMWithTasks *
cora_vm_with_tasks_create(void) {
    CoraVMWithTasks *vm = malloc(sizeof(CoraVMWithTasks));
    if (!vm) {
        return NULL;
    }
    
    // Initialize the underlying Cora VM
    vm->cora_vm = coraInit();
    if (!vm->cora_vm) {
        free(vm);
        return NULL;
    ...
    vm->ready_tasks = Nil;  // Empty task list
    pthread_mutex_init(&vm->task_queue_lock, NULL);
    vm->total_tasks_spawned = 0;
...
void
cora_vm_with_tasks_destroy(CoraVMWithTasks *vm) {
    if (!vm) {
        return;
    }
    
    // Clean up underlying Cora VM
    if (vm->cora_vm) {
        coraExit(vm->cora_vm);
        vm->cora_vm = NULL;
    }
    ...
    // Clean up task queue
    pthread_mutex_destroy(&vm->task_queue_lock);
...
void
cora_vm_spawn_task(CoraVMWithTasks *vm, Obj thunk) {
    if (!vm || !thunk) {
        return;
    }
    
    pthread_mutex_lock(&vm->task_queue_lock);
    
    // Add task to ready queue (prepend to list)
    vm->ready_tasks = makeCons(vm->cora_vm->gc, thunk, vm->ready_tasks);
    vm->total_tasks_spawned++;
    
    pthread_mutex_unlock(&vm->task_queue_lock);
}

// Check if there are ready tasks
bool
cora_vm_has_ready_tasks(CoraVMWithTasks *vm) {
    if (!vm) {
        return false;
    }
    
    pthread_mutex_lock(&vm->task_queue_lock);
    bool has_tasks = (vm->ready_tasks != Nil);
    pthread_mutex_unlock(&vm->task_queue_lock);
...
void
cora_vm_run_one_task(CoraVMWithTasks *vm) {
    if (!vm) {
        return;
    }
    ...
    // Atomically get one task from the queue
    pthread_mutex_lock(&vm->task_queue_lock);
    if (vm->ready_tasks != Nil) {
        task_thunk = car(vm->ready_tasks);
        vm->ready_tasks = cdr(vm->ready_tasks);
    }
    pthread_mutex_unlock(&vm->task_queue_lock);
    
    if (task_thunk != Nil) {
        // For now, we'll skip execution due to the thread-safety issue with coraRun()
        // In a real implementation, we would execute the task:
        /*
        Cora *cora_vm = vm->cora_vm;
        coraCall0(cora_vm, task_thunk);
        coraRun(cora_vm);
        */
        
        // For now, just acknowledge the task was processed
        (void)task_thunk; // Suppress unused variable warning
    }
}

// Get the underlying Cora VM instance
Cora *
cora_vm_get_cora_instance(CoraVMWithTasks *vm) {
    if (!vm) {
        return NULL;
    }
    return vm->cora_vm;
